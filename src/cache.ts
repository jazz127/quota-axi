 ...
 * OpenRouter, and Muse, whose identity a reading always has (and which skip write and clear
...
export function writeCachedProviders(
  ...
  withCacheWriteLock(() => {
    ...
    const cacheable = providers
      .map((provider) => {
        const existing = existingByProvider.get(cacheIdentity(provider));
        const record = toCacheProvider(
          preservesOpenRouter(provider) && provider.credits === undefined
            ? { ...provider, credits: existing?.snapshot.credits }
            : provider,
        );
