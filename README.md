...
- **Validation boundary:** deterministic tests cover the credential and failure matrix. Separately authorized native runs returned fresh numeric quota for a selected public/default-profile account on macOS (CLI 1.0.87-0) and Windows x64 on JI7 (CLI 1.0.86). The Windows proof used the built provider adapter with exactly one Credential Manager read and one quota GET, which returned HTTP 200 and three numeric windows. [Windows evidence](data/quota-axi-copilot-windows-ji7-r2/windows-evidence.md) records its scope. This does not validate other accounts, selectors, CLI versions, architectures, sessions, platforms, or universal quota entitlement.
...
- Native environment selection is deliberately unsupported. Presence of `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_TOKEN`, `COPILOT_GH_HOST`, or `GH_HOST` makes the native source report `environment_selection_unsupported`; their credential values are never read. This source does not claim to reproduce the CLI's environment-selected active account.
- `gh:hosts.yml` still reads only the public `github.com` host's own `oauth_token`, never nested per-user or enterprise tokens, the gh keyring, or a launched `gh` command. Unsupported gh keyring storage remains a skipped attempt; if no source answers and no earlier native-source diagnostic takes precedence, it produces `unavailable` with `credentials_keyring_storage` rather than asserting sign-out. An unreadable/unparseable gh file retains its existing skipped-source behavior.
- A retrieved token proves source support, not quota entitlement. It is used only as a bearer for `https://api.github.com/copilot_internal/user`, with redirects rejected. Native 401/non-rate-limit 403 permits credential fallback; if no source answers, the report says the credential was rejected or quota access denied rather than instructing a scope or login change. An accepted response without numeric windows remains fresh with `windows: []`. No provider login or refresh is performed.
- Native readings use top-level source `cli` and their exact source name in attempts. Native snapshots are never reused as stale, and native config that exists or whose absence cannot be confirmed also withholds old legacy snapshots, avoiding another account's quota after selection changes. Legacy cache behavior remains when native metadata is absent.
