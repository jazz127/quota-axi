...
- Native readings use top-level source `cli` and their exact source name in attempts. Native snapshots are never reused as stale, and a present native selection also withholds old legacy snapshots, avoiding another account's quota after selection changes. Legacy cache behavior remains when native metadata is absent.
