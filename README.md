...
| `credits`                                               | `all`   | The provider reported a raw credit balance, or OpenRouter reported that its account balance was unavailable. `detail` states the balance or reason; no percentage or bound is derived from it. For OpenRouter this may accompany measurable scopes. |
...
| OpenRouter             | Reads a local OpenRouter/Pi API key from the first-party `/api/v1/key` endpoint and, when permitted, `/api/v1/credits`. A finite key cap is reported as a raw `key-limit` amount; capless keys retain the endpoint's uncapped `usage`, `usage_daily`, `usage_weekly`, and `usage_monthly` spend meters, while the account balance is `total_credits - total_usage` when available. `free_model_daily_requests` is reported as a daily window. A refused or malformed credits response is not a provider failure, and capless readings never claim an unlimited account or infer model headroom.                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- After the explicit `CLAUDE_CODE_OAUTH_TOKEN` and Claude Code OAuth-file sources, quota-axi can read Pi's `anthropic` OAuth entry from `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`) as source `pi:anthropic`. The Pi entry is read-only: its `access` and `expires` fields are used for the bounded Anthropic quota probe, expiry remains advisory, and `refresh` is presence-only so quota-axi never exchanges or logs it. A missing Pi entry is silent; malformed, unsupported, or expired entries remain visible in full source evidence and follow the normal Claude credential handover and expiry rules.
...
- In `--full` output, Claude usage attempts identify `oauth-file`, `pi:anthropic`, or `keychain` as the credential discovery source. They never include the Keychain account or Pi credential material.
...
- It sends a redirect-disabled `GET` per attempted credential to OpenRouter's first-party `/api/v1/key` endpoint with a bearer token, then separately reads `/api/v1/credits` for the account balance when that operation permits the key. It tries the environment key before Pi under the [credential handover rules](#provider-state). Key-cap representation, uncapped spend meters, and the free-model daily window are documented under [Provider windows](#provider-windows); reset text comes only from the endpoint. The endpoint's credential-derived `data.label` is intentionally not exposed as account identity or on any other output or cache surface.
- Capless keys never become `credits.unlimited`: when `/credits` cannot report the account balance, the fresh report says that the key has no spend cap and the account balance was not reported. The cache is scoped by a one-way digest of the answering credential and source, so a later Pi profile or rotated key cannot reuse another reading; malformed or unexpected responses leave an existing cache entry untouched.
