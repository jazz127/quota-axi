...
- Native readings use top-level source `cli` and their exact source name in attempts. Native snapshots are never reused as stale, and a native source that could have answered - config that exists on a supported platform, or whose absence cannot be confirmed - also withholds old legacy snapshots, avoiding another account's quota after selection changes. Legacy cache behavior remains when native metadata is absent or the platform has no supported secure store.
