...
| `--allow-keychain-prompt`                                                                  | Permit native secure-store access (macOS or Copilot on Windows)           |
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `copilot-cli:keychain`, `gh:hosts.yml`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:openai-codex-*` sibling keys, `pi:kimi-coding`, `pi:xai`, `pi:zai`, `pi:commandcode`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, `loopback`, `env:COMMAND_CODE_API_KEY`, `env:COMMANDCODE_API_KEY`, `commandcode-cli`, and `omp:commandcode` |
...
| GitHub Copilot | See [Copilot provider notes](#provider-notes) for credential sources, supported native backends, and consent boundaries.                                                                                                                                                                                                                                                                                                                                                     |
...
- The added native source is secure-store-only. It does not read the CLI’s plaintext token fallback or recommend moving credentials into plaintext. Existing legacy apps and gh file behavior is preserved; standalone CLI file-store support is outside this addition.
- **Validation boundary:** deterministic tests cover the credential and failure matrix. Separately authorized native runs returned fresh numeric quota for a selected public/default-profile account on macOS (CLI 1.0.87-0) and Windows x64 on JI7 (CLI 1.0.86). The Windows proof used the built provider adapter with exactly one Credential Manager read and one quota GET, which returned HTTP 200 and three numeric windows. This does not validate other accounts, selectors, CLI versions, architectures, sessions, platforms, or universal quota entitlement.
- Sources are tried in ownership order: legacy `apps.json`, native `copilot-cli:keychain`, then GitHub CLI `gh:hosts.yml`. A working apps source retains its `api` attempt name. Handover happens after absent/unusable credentials or non-rate-limit HTTP 401/403; network, decoding, server, and rate-limit failures stop handover. Each failed source remains visible when a sibling answers. These are independent credential owners: a working apps source or a later gh fallback can represent a different account from the native CLI selection. Only a successful native source is bound to that selected identity; the provider does not reproduce the CLI’s full active-session authentication precedence.
- Legacy `apps.json` uses `$GITHUB_COPILOT_APPS_JSON` when set; otherwise it uses `$LOCALAPPDATA/github-copilot/apps.json` on Windows (default `~/AppData/Local`) or `${XDG_CONFIG_HOME:-~/.config}/github-copilot/apps.json` elsewhere. GitHub CLI `hosts.yml` uses `$GH_CONFIG_DIR`, then `$XDG_CONFIG_HOME/gh`, then `%AppData%\GitHub CLI` on Windows when set, otherwise `~/.config/gh`.
- The native source supports the default `~/.copilot/config.json` profile on macOS and Windows (`%USERPROFILE%\.copilot\config.json`) with an exact `lastLoggedInUser` selection whose host is `https://github.com`. macOS reads service `copilot-cli` and account `<selected host>:<selected login>`. Windows reads exactly one `CRED_TYPE_GENERIC` (type 1) item with target `<selected host>:<selected login>.copilot-cli` and username `<selected host>:<selected login>`. The `LegacyGeneric:target=` prefix shown by `cmdkey` is display notation, not part of the target passed to `CredReadW`. The historical source name `copilot-cli:keychain` covers both native backends. The reader never selects the first available user, reads a plaintext `copilotTokens` fallback, launches a provider, refreshes credentials, or migrates storage. The selected identity is checked again after a value read before the token is used.
- The macOS binding and literal token payload were empirically checked with CLI **1.0.87-0**; the Windows binding was checked on JI7 with CLI **1.0.86**. They are not a stable vendor contract or a claim about every CLI version. GitHub's [authentication documentation](https://github.com/github/docs/blob/733a8a36df30c11712cdf691a5a9296b669172d5/content/copilot/how-tos/copilot-cli/set-up-copilot-cli/authenticate-copilot-cli.md) documents the service and multiple-account behavior but not the exact item schema. Different host spellings, enterprise selections, absent/malformed selections, and non-default `COPILOT_HOME` profiles are explicitly unsupported for secure retrieval. `COPILOT_HOME` determines where metadata is inspected; no default profile is substituted. Linux and other unsupported platforms report `secure_store_unsupported` after native metadata yields a syntactically valid selected identity; missing, unreadable, or invalid metadata retains its own diagnostic. Existing apps and gh file sources remain available on their supported platforms.
- Plain macOS calls inspect item metadata only until `--allow-keychain-prompt` succeeds. On Windows, plain calls inspect only the CLI config selection: `CredReadW` returns the secret with metadata, so it is never used as a presence probe. The same opt-in flag permits Windows access; it does not promise an OS prompt. A non-secret marker, hashed over the config path, service, and account, permits later quota reads for that same selection (`0600` on POSIX; inherited filesystem ACLs on Windows). Ordinary `auth` stays metadata-only even with a marker; explicit `--allow-keychain-prompt auth` may bootstrap access. Missing config, invalid config, unsupported selection, inaccessible item, denial, timeout, and unsupported token format remain distinct. Exit 44 means `keychain_item_unavailable`, not proof of sign-out. Only `keychain_prompt_required` offers the prompt remedy. No credential value is rendered or persisted.
- Windows requires the selected user's accessible Credential Manager logon session and Windows PowerShell. The helper uses a fixed system PowerShell path, no profile, noninteractive execution, a 10-second timeout, and bounded captured output. It checks the returned target, username, and type, accepts only a bounded UTF-16 literal supported GitHub token, and frees the native buffer. `credential_not_found`, `credential_logon_session_unavailable`, `credential_access_denied`, `credential_binding_mismatch`, `credential_format_unsupported`, `credential_read_timeout`, and `credential_read_failed` remain distinct unmeasured outcomes. They never imply sign-out or trigger store enumeration, another native item lookup, login, or plaintext migration. Inaccessible service/SSH sessions cannot establish the desktop user's absence. Windows ARM64 and other session configurations remain unqualified.
- Native environment selection is deliberately unsupported. A non-blank `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_TOKEN`, `COPILOT_GH_HOST`, or `GH_HOST` makes the native source report `environment_selection_unsupported`; their credential values are never read, and a blank value selects nothing and preserves the stored path. This source does not claim to reproduce the CLI's environment-selected active account.
- `gh:hosts.yml` still reads only the public `github.com` host's own `oauth_token`, never nested per-user or enterprise tokens, the gh keyring, or a launched `gh` command. Unsupported gh keyring storage remains a skipped attempt and retains its established verdict. An unreadable/unparseable gh file retains its existing skipped-source behavior.
- A retrieved token proves source support, not quota entitlement. It is used only as a bearer for `https://api.github.com/copilot_internal/user`, with redirects rejected. Native 401/non-rate-limit 403 permits credential fallback; the rejected native attempt says the credential was rejected or quota access denied rather than instructing a scope or login change, and every source being definitively rejected still reports sign-in required. An accepted response without numeric windows remains fresh with `windows: []`. No provider login or refresh is performed.
- Native readings use top-level source `cli` and their exact source name in attempts. Native snapshots are never written to the cache or reused as stale, and a native source that could have answered - config that selects an account on a supported platform, or whose contents cannot be confirmed - also withholds old legacy snapshots, avoiding another account's quota after selection changes. Legacy cache behavior remains when native metadata is absent, selects no account, or the platform has no supported secure store; the same reasons also keep the native source from defining the published verdict.
- Selected-account, bounded-file, comment/URL, and malformed-config test coverage adapts the ideas in [adibirzu's original Copilot contribution](https://github.com/kunchenguid/quota-axi/pull/50), retaining the repository's MIT license. The old plaintext reader, unsafe first-public-account fallback, and Cursor changes are not included.
