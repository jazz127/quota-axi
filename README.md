...
- `gh:hosts.yml` still reads only the public `github.com` host's own `oauth_token`, never nested per-user or enterprise tokens, the gh keyring, or a launched `gh` command. Unsupported gh keyring storage remains a skipped attempt but, if no source answers, now produces `unavailable` with `credentials_keyring_storage` rather than asserting sign-out. An unreadable/unparseable gh file retains its existing skipped-source behavior.
