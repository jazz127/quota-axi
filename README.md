...
- **Validation boundary:** deterministic tests pass, but the final native secure-store implementation has not been validated end to end against a real Keychain and quota endpoint (0 of 5 live scenarios completed). The earlier single native observation is limited historical evidence of the item binding and payload, not validation of this implementation, other selectors or platforms, or universal quota entitlement.
- Sources are tried in ownership order: legacy `apps.json`, native `copilot-cli:keychain`, then GitHub CLI `gh:hosts.yml`. A working apps source retains its `api` attempt name. Handover happens after absent/unusable credentials or non-rate-limit HTTP 401/403; network, decoding, server, and rate-limit failures stop handover. Each failed source remains visible when a sibling answers.
