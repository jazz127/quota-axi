...
- For Codex, Grok, Command Code, and OpenRouter, a fresh positive raw credit balance, or an explicitly unlimited balance, appears on the provider card as `balance · ...` beside its measured windows. It is display-only: zero, stale, and failed readings omit the line, and no percentage or bound is derived from it. A per-key spend-cap window remains the source of its own row rather than being duplicated as a balance; other providers retain their provider-specific balance behavior.
