...
- A rejected Copilot CLI token reports `GitHub Copilot credential rejected or quota access denied` on its attempt and hands over to the `gh` login. Copilot CLI readings are never written to the cache or reused as stale. When that source could have answered (its config selects an account on a supported platform and is not only awaiting `--allow-keychain-prompt`, or the config cannot be read with certainty), an older `apps.json` or `gh` snapshot is withheld too, so another account's quota is never served after the selection changes. A missing config, no selection, an unsupported platform, or an item only awaiting the opt-in keeps the existing stale fallback, and also keeps the Copilot CLI source from defining the verdict.
