...
- **No first-run Keychain prompt** - macOS Claude, Cursor CLI, and Copilot CLI Keychain value reads are skipped on plain calls until `--allow-keychain-prompt` succeeds once for that source, then future plain calls reuse the corresponding grant.
...
- **Validation boundary:** deterministic tests cover the credential and failure matrix. One separately authorized end-to-end run of this implementation returned fresh numeric quota for a selected public account using the default macOS profile and the empirically observed CLI 1.0.87-0 binding. The wider live matrix remains unqualified: this does not validate other accounts, selectors, CLI versions, platforms, secure stores, or universal quota entitlement. Earlier item-binding observations alone were not end-to-end implementation validation.
...
- The native source initially supports macOS's default `~/.copilot/config.json` profile with an exact `lastLoggedInUser` selection whose host is `https://github.com`. It looks up only service `copilot-cli` and account `<selected host>:<selected login>`, with fixed subprocess arguments and bounded output/time. It never selects the first available user, reads a plaintext `copilotTokens` fallback, launches a provider, refreshes credentials, or migrates storage. The selected identity is checked again after a value read before the token is used.
- This binding and a literal token payload were empirically checked with CLI **1.0.87-0**. They are not a stable vendor contract or a claim about every CLI version. GitHub's [authentication documentation](https://github.com/github/docs/blob/733a8a36df30c11712cdf691a5a9296b669172d5/content/copilot/how-tos/copilot-cli/set-up-copilot-cli/authenticate-copilot-cli.md) documents the service and multiple-account behavior but not the exact item schema. Different host spellings, enterprise selections, absent/malformed selections, and non-default `COPILOT_HOME` profiles are explicitly unsupported for secure retrieval. `COPILOT_HOME` determines where metadata is inspected; no default profile is substituted. Other platforms report `secure_store_unsupported` after native metadata yields a syntactically valid selected identity; missing, unreadable, or invalid metadata retains its own diagnostic. Existing apps and gh file sources remain available on their supported platforms.
- Plain calls inspect item metadata only until `--allow-keychain-prompt` succeeds. A non-secret `0600` marker, hashed over the config path, service, and account, permits later quota reads for that same selection. Ordinary `auth` stays metadata-only even with a marker; explicit `--allow-keychain-prompt auth` may bootstrap access. Missing config, invalid config, unsupported selection, inaccessible item, denial, timeout, and unsupported token format remain distinct. Exit 44 means `keychain_item_unavailable`, not proof of sign-out. Only `keychain_prompt_required` offers the prompt remedy. No credential value is rendered or persisted.
