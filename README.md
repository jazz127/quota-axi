...
OpenCode Go balance note: when the usage payload carries an explicit `balance` object with numeric `usd` and optional `currency: "USD"`, quota-axi publishes it as top-level `credits.remaining` with `unit: "usd"`. Absent or differently typed balance data stays absent. This evidence does not change effective availability, runway, selection, exhaustion ranking, or compact ranking. The public route does not currently document this shape, so raw integer balances and guessed aliases are not mapped.
